<template>
    ...
    <v-chip-group
        v-if="props.num_length > 100"
        v-model="lottoSelected"
        selected-class="bg-primary"
        column
        multiple
        class="lottery-bet-chips mb-5 pb-5"
    >
        <v-tabs
            v-model="numberTab"
            align-tabs="center"
            color="deep-purple-accent-4"
            class="tab-chunk-100 mb-4"
        >
            <v-tab 
                v-for="f in numbers.length"
                :value="f-1"><span class="font-weight-bold">{{ (f-1).toString().padEnd(props.pad_start, '0') }}</span>
            </v-tab>
        </v-tabs>
        <v-window v-model="numberTab">
            <v-window-item
                v-for="(ns, i) in numbers"
                :key="i"
                :value="i"
            >
                <v-chip
...
const props = defineProps(['type', 'num_length', 'chunk', 'pad_start'])
...
const number = ref(Array.from({length: props.num_length}, (_, index) => index.toString().padStart(props.pad_start, '0')))
...
async function updateChunk() {
    for (let i = 0; i < number.value.length; i += props.chunk) {
        const chunk = number.value.slice(i, i + props.chunk)
...
async function updateLotteryNumber() {
    lottoSelected.value = []
    store.lottoNumber.forEach((n) => { lottoSelected.value.push(n.number) })

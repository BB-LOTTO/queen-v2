...
const number = ref(Array.from({length: 10}, (_, index) => index.toString().padStart(2, '0')))
...
async function updateLotteryRunNumber() {
    lottoSelected.value = []
...
function numberSearch() {
    if(search_number.value.length > 0) {
        run_chips.value.forEach((chip, index) => {
